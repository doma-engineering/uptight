defmodule Uptight.Assertions.Formatter do
  @moduledoc """
  Helper functions for formatting and the formatting protocols.

  Formatters are `GenServer`s specified during Uptight configuration
  ...
  The following events are possible:
    ...
    * `{:module_started, test_module}` -
      a test module has started. See `Uptight.TestModule` for details.

    * `{:module_finished, test_module}` -
      a test module has finished. See `Uptight.TestModule` for details.

    * `{:test_started, test}` -
      a test has started. See `Uptight.Test` for details.

    * `{:test_finished, test}` -
      a test has finished. See `Uptight.Test` for details.
  ...
  @formatter_exceptions [Uptight.AssertionError, FunctionClauseError]
  @no_value Uptight.AssertionError.no_value()
  ...
  @doc false
  def format_assertion_error(%Uptight.AssertionError{} = struct) do
    format_exception(%{}, struct, [], :infinity, fn _, msg -> msg end, "") |> elem(0)
  end

  defp format_exception(test, %Uptight.AssertionError{} = struct, stack, width, formatter, pad) do
  ...
  @doc false
  @deprecated "Use Uptight.Assertions.Formatter.format_test_all_failure/5 instead"
