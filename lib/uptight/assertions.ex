defmodule Uptight.AssertionError do
  @moduledoc """
  ...
  def message(exception) do
    "\n\n" <> Uptight.Assertions.Formatter.format_assertion_error(exception)
  end
end
...
defmodule Uptight.Assertions do
  @moduledoc """
  ...
  defmacro assert({:match?, meta, [left, right]} = assertion) do
    ...
    quote do
      ...
      Uptight.Assertions.assert(unquote(match?),
        right: right,
        left: left,
        expr: unquote(code),
        message: "match (match?) failed" <> Uptight.Assertions.__pins__(unquote(pins)),
      ...
      refute unquote(match?),
        ...
        message:
          "match (match?) succeeded, but should have failed" <>
            Uptight.Assertions.__pins__(unquote(pins)),
        context: {:match, unquote(pins)}
    end
  end
  ...
  defp translate_operator(kind, {op, meta, [left, right]} = expr, call, message, true, _caller) do
    ...
    quote do
      ...
      if Uptight.Assertions.__equal__?(left, right) do
        Uptight.Assertions.assert(false,
          left: left,
          expr: unquote(expr),
          message: unquote(message <> ", both sides are exactly equal")
        )
      else
        Uptight.Assertions.assert(unquote(call),
          left: left,
          right: right,
          expr: unquote(expr),
  ...
  defp extract_args({root, meta, [_ | _] = args} = expr, env) do
    ...
    case Macro.expand_once(expr, env) do
      ...
      other ->
        {Uptight.AssertionError.no_value(), other}
    end
  end

  defp extract_args(expr, _env) do
    {Uptight.AssertionError.no_value(), expr}
  end
          ...
          case right do
            ...
            _ ->
              ...
              raise Uptight.AssertionError,
                ...
                right: right,
                expr: expr,
                message: "match (=) failed" <> Uptight.Assertions.__pins__(unquote(pins)),
                context: {:match, unquote(pins)}
          end
        end
      )
  ...
  def assert(value, opts) when is_list(opts) do
    unless value, do: raise(Uptight.AssertionError, opts)
    true
  end
  ...
  defp assert_receive(pattern, timeout, failure_message, caller, code) do
    ...
    failure_message =
      failure_message ||
        quote do
          Uptight.Assertions.__timeout__(
    ...
    quote do
      timeout = Uptight.Assertions.__timeout__(unquote(timeout), :assert_receive_timeout)
      ...
      raise Uptight.AssertionError,
        ...
        This means the message was delivered too close to the timeout value, you may want to either:
          ...
          2. Increase the default timeout to all `assert_receive` in your
             test_helper.exs by setting Uptight.configure(assert_receive_timeout: ...)
        """
    else
      ...
      raise Uptight.AssertionError,
        ...
        message:
          "Assertion failed, no matching message after #{timeout}ms" <>
            Uptight.Assertions.__pins__(pins) <> message,
      ...
      error ->
        ...
        cond do
          ...
          true ->
            ...
            reraise Uptight.AssertionError, [message: message], __STACKTRACE__
        end
    else
  ...
  defp do_refute_receive(pattern, timeout, failure_message) do
    ...
    quote do
      timeout = Uptight.Assertions.__timeout__(unquote(timeout), :refute_receive_timeout)
